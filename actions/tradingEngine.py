...
class TradingEngine:
    ...
    def check_for_trading_action(self, token_address):
        ...
        if self.active_position:
            ...
            if self.check_if_sell_signal(self.active_position, current_price):
                ...
                if self.portfolio.sell(token_address, self.active_position["amount"], current_price, slippage=0.02):
                    action = "SOLD"
                ...
                additional_amount = self.calculate_buy_amount(current_price, slippage_percentage=0.02)
                if additional_amount > 0 and self.portfolio.buy(token_address, current_price, additional_amount, slippage=0.02):
                    ...
                    adjusted_price = current_price * (1 + 0.02)
            ...
            if self.check_if_buy_signal(current_price):
                buy_amount = self.calculate_buy_amount(current_price, slippage_percentage=0.02)
                if buy_amount > 0 and self.portfolio.buy(token_address, current_price, buy_amount, slippage=0.02):
    ...
    def calculate_buy_amount(self, current_price, slippage_percentage):
        available_balance = self.portfolio.holdings["USDC"]
        buy_percentage = 0.1  # 10% of available USDC balance
    ...
    def calculate_stop_loss_range(self, current_price):
        # For example, set a stop loss zone as a range between 5% and 3% below the current price
        lower_bound = current_price * 0.95
        upper_bound = current_price * 0.97
        return (lower_bound, upper_bound)

    def calculate_take_profit(self, current_price):
        # For example, take profit at 10% above the current price
