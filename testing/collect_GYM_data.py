from analytics.metric_collector import TestDataCollector
from actions.tradingEngine import TradingEngine
from utils.os_utils import fetch_complete_test_data, load_historical_data_from_file, save_historical_data_to_file, save_metrics_to_csv
import random
...
if __name__ == "__main__":
    ...
    for token in token_list:
        ...
        try:
            historical_data = load_historical_data_from_file(token, REFRESH_INTERVAL, FETCHING_SPAN_IN_DAYS, ohlcv=OHLCV,
        ...
        if not historical_data:
            historical_data = fetch_complete_test_data(token, REFRESH_INTERVAL, FETCHING_SPAN_IN_DAYS, ohlcv=False)
    ...
    starting_index = random(10, 150) # simulate starting index for data collection
    ...
    for i in range( starting_index, len(historical_data["data"]["items"])):
        ...
        TradingEngine.collect_metrics_for_current_point()
        


    collector = TestDataCollector(REFRESH_INTERVAL, historical_data)  # initializes data collector with ALL historical data
    token_metrics =collector.collect_data() # iterates through historic data of a token in a simulated real-time feed 
    print(token_metrics)
